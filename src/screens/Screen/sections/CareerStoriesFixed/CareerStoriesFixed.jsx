
import 
 
  React 

 

from
"react"
import 

 
 { 
  
    Btn  
    
   }

from
"../../../../components/Btn"
import 

 
 { 
  
    SectionHeader  
    
   }

from
"../../../../components/SectionHeader"
import 

 
 { 
  
    StoryCard  
    
   }

from
"../../../../components/StoryCard"

export const CareerStoriesFixed = () => {
  

  return (
    <div
  
    className= "flex flex-col w-full max-w-[1324px] items-center gap-8 md:gap-14 p-4 md:p-10 relative flex-[0_0_auto] bg-grey1-bg" 

  

  

  

  


>
<div
  
    className= "flex flex-col items-center gap-12 relative self-stretch w-full flex-[0_0_auto]" 

  

  

  

  


>
<div
  
    className= "h-auto md:h-[189px] gap-10 md:gap-10 flex flex-col items-center relative self-stretch w-full" 

  

  

  

  


>
<SectionHeader
  
    
        className="!self-stretch !h-[58.73px] !left-[unset] !w-full !top-[unset]"  
      
    
        h1="転職成功事例"  
      
    
        prop="XXXXの方々の転職成功ストーリーをご紹介"  
      
    
  
/>
<div
  
    className= "inline-flex flex-wrap items-center justify-center gap-4 md:gap-[31px] relative flex-[0_0_auto] mb-6 md:mb-[-0.73px] px-4 md:px-0" 

  

  

  

  


>
<div
  
    className= "relative w-[160px] md:w-48 h-[70px] md:h-[91px] flex-shrink-0" 

  

  

  

  


>
<div
  
    className= "flex flex-col w-[130px] md:w-[138px] h-[56px] md:h-[76px] items-center gap-[6px] md:gap-[10.87px] absolute top-[10px] md:top-[15px] left-[15px] md:left-[27px]" 

  

  

  

  


>
<div
  
    className= "inline-flex items-baseline justify-center relative flex-[0_0_auto]" 

  

  

  

  


>
<div
  
    className= "w-fit mt-[-1.36px] bg-[linear-gradient(90deg,rgba(137,185,41,1)_0%,rgba(87,195,132,1)_50%,rgba(17,165,177,1)_100%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] [font-family:'Open_Sans',Helvetica] font-bold text-transparent text-[28px] md:text-[43px] text-center leading-[28px] md:leading-[43px] whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
1,247
</div>
<div
  
    className= "w-fit bg-[linear-gradient(90deg,rgba(137,185,41,1)_0%,rgba(87,195,132,1)_50%,rgba(17,165,177,1)_100%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] [font-family:'Noto_Sans_JP',Helvetica] font-bold text-transparent text-sm md:text-2xl text-center leading-[14px] md:leading-6 whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
名
</div>
</div>
<div
  
    className= "w-fit [font-family:'Noto_Sans_JP',Helvetica] font-medium text-text text-sm md:text-[22px] text-center leading-[14px] md:leading-[22px] whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
転職成功者数
</div>
</div>
<img
  
    className= "absolute -top-px -left-px w-[161px] md:w-[193px] h-[71px] md:h-[84px]" 

  

  

  alt="Vector"
src="/img/vector-7.svg"


  


/>
</div>
<div
  
    className= "relative w-[170px] md:w-[200px] h-[70px] md:h-[91px] flex-shrink-0" 

  

  

  

  


>
<div
  
    className= "flex flex-col w-[145px] md:w-[173px] h-[56px] md:h-[76px] items-center gap-[6px] md:gap-[10.87px] absolute top-[10px] md:top-[15px] left-[12px] md:left-3.5" 

  

  

  

  


>
<div
  
    className= "inline-flex items-baseline justify-center relative flex-[0_0_auto]" 

  

  

  

  


>
<div
  
    className= "w-fit bg-[linear-gradient(90deg,rgba(137,185,41,1)_0%,rgba(87,195,132,1)_50%,rgba(17,165,177,1)_100%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] [font-family:'Noto_Sans_JP',Helvetica] font-bold text-transparent text-sm md:text-2xl text-center leading-[14px] md:leading-6 whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
平均
</div>
<div
  
    className= "relative w-fit mt-[-1.36px] bg-[linear-gradient(90deg,rgba(137,185,41,1)_0%,rgba(87,195,132,1)_50%,rgba(17,165,177,1)_100%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] font-bold text-[30px] md:text-[43px] text-center leading-[30px] md:leading-[43px] [font-family:'Open_Sans',Helvetica] text-transparent tracking-[0] whitespace-nowrap" 

  

  

  

  


>
142
</div>
<div
  
    className= "w-fit bg-[linear-gradient(90deg,rgba(137,185,41,1)_0%,rgba(87,195,132,1)_50%,rgba(17,165,177,1)_100%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] [font-family:'Noto_Sans_JP',Helvetica] font-bold text-transparent text-sm md:text-2xl text-center leading-[14px] md:leading-6 whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
万円
</div>
</div>
<div
  
    className= "w-fit [font-family:'Noto_Sans_JP',Helvetica] font-bold text-text text-sm md:text-[22px] text-center leading-[14px] md:leading-[22px] whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
年収アップ額
</div>
</div>
<img
  
    className= "absolute -top-px -left-px w-[171px] md:w-[201px] h-[71px] md:h-[84px]" 

  

  

  alt="Vector"
src="/img/vector-8.svg"


  


/>
</div>
<div
  
    className= "relative w-[160px] md:w-48 h-[70px] md:h-[91px] flex-shrink-0" 

  

  

  

  


>
<div
  
    className= "flex flex-col w-[130px] md:w-28 h-[56px] md:h-[76px] items-center gap-[6px] md:gap-[10.87px] absolute top-[10px] md:top-[15px] left-[15px] md:left-[41px]" 

  

  

  

  


>
<div
  
    className= "inline-flex items-baseline justify-center relative flex-[0_0_auto]" 

  

  

  

  


>
<div
  
    className= "w-fit mt-[-1.36px] bg-[linear-gradient(90deg,rgba(137,185,41,1)_0%,rgba(87,195,132,1)_50%,rgba(17,165,177,1)_100%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] [font-family:'Open_Sans',Helvetica] font-bold text-transparent text-[28px] md:text-[43px] text-center leading-[28px] md:leading-[43px] whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
96.8
</div>
<div
  
    className= "w-fit bg-[linear-gradient(90deg,rgba(137,185,41,1)_0%,rgba(87,195,132,1)_50%,rgba(17,165,177,1)_100%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] [font-family:'Noto_Sans_JP',Helvetica] font-bold text-transparent text-sm md:text-2xl text-center leading-[14px] md:leading-6 whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
%
</div>
</div>
<div
  
    className= "w-fit [font-family:'Noto_Sans_JP',Helvetica] font-medium text-text text-sm md:text-[22px] text-center leading-[14px] md:leading-[22px] whitespace-nowrap relative tracking-[0]" 

  

  

  

  


>
転職満足度
</div>
</div>
<img
  
    className= "absolute -top-px -left-px w-[161px] md:w-[193px] h-[71px] md:h-[84px]" 

  

  

  alt="Vector"
src="/img/vector-9.svg"


  


/>
</div>
</div>
</div>
<div
  
    className= "flex md:grid overflow-x-auto md:overflow-x-visible md:grid-cols-3 gap-4 md:gap-6 relative self-stretch w-full snap-x snap-mandatory scrollbar-hide mt-0 md:mt-0" 

  

  

  

  


>
<StoryCard
  
    
        text1="年収アップと安定した労働環境"  
      
    
        text3="テキストがはいります"  
      
    
        afterAfterClassName="!-ml-2.5 bg-[url(/img/union-6.svg)] !left-[unset] !top-[unset]"  
      
    
        avatarElement="/img/avatar-2.svg"  
      
    
        className="!mt-[-0.50px] !mb-[-0.50px] !left-[unset] !top-[unset] min-w-[85vw] md:min-w-0 snap-start"  
      
    
        prop="フォークリフトスタッフ"  
      
    
  
/>
<StoryCard
  
    
        text1="年収アップと安定した労働環境"  
      
    
        text3="テキストがはいります"  
      
    
        afterAfterClassName="!-ml-2.5 bg-[url(/img/union-8.svg)] !left-[unset] !top-[unset]"  
      
    
        avatarImg="/img/avatar-3.svg"  
      
    
        avatarOne="two"  
      
    
        className="!mt-[-0.50px] !mb-[-0.50px] !left-[unset] !top-[unset] min-w-[85vw] md:min-w-0 snap-start"  
      
    
        prop="XXXスタッフ"  
      
    
        selectedTag2="実務経験 4年"  
      
    
        text="平田 さん（30歳）"  
      
    
  
/>
<StoryCard
  
    
        text1="年収アップと安定した労働環境"  
      
    
        text3="テキストがはいります"  
      
    
        afterAfterClassName="!-ml-2.5 bg-[url(/img/union-10.svg)] !left-[unset] !top-[unset]"  
      
    
        avatarElement1="/img/avatar-4.svg"  
      
    
        avatarOne="three"  
      
    
        className="!mt-[-0.50px] !mb-[-0.50px] !left-[unset] !top-[unset] min-w-[85vw] md:min-w-0 snap-start"  
      
    
        prop="XXXスタッフ"  
      
    
        selectedTag2="実務経験 4年"  
      
    
        text="平田 さん（37歳）"  
      
    
  
/>
</div>
</div>
<Btn
  
    
        text2="他の転職事例を見る"  
      
    
        className="!left-[unset] !w-[308px] !top-[unset]"  
      
    
        iconLeft="off"  
      
    
        iconRight="on"  
      
    
        state="default"  
      
    
        to="/u36578u32887u20107u20363u12504u12442u12540u12471u12441"  
      
    
        type="fill"  
      
    
  
/>
</div>
)
}



